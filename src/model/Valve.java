...
public class Valve { // Declaración de la clase pública Valve que representa una entidad en la simulación de cerveza
    public enum Type { // Enumeración pública que define los tipos de entidades en el proceso cervecero
        GRANOS_CEBADA("Granos Cebada", Color.decode("#D4A574"), new String[]{"SILO_GRANDE", "MALTEADO", "SECADO", "MOLIENDA", "MACERADO", "FILTRADO", "COCCION"}), // Granos color café claro
        ...
        private final String[] route; // Ruta de procesamiento: array de nombres de locaciones

        Type(String displayName, Color color, String[] route) { // Constructor del enum Type que inicializa los atributos
        ...
        public String[] getRoute() { return route; } // Método getter que retorna la ruta de procesamiento
    }
    ...
    public static final Map<String, Double> LOCATION_PROCESS_TIMES = initializeProcessTimes(); // Mapa estático PÚBLICO de tiempos de proceso por locación

    private final int id; // ID único de la entidad (final porque no cambia)
    private final Type type; // Tipo de entidad (final porque no cambia)
    private final double arrivalTime; // Tiempo de arribo de la entidad al sistema en minutos (final porque no cambia)
    private State state; // Estado actual de la entidad
    private Location currentLocation; // Locación actual donde se encuentra la entidad
    ...
    public String getNextLocation() { // Método que retorna el nombre de la próxima locación en la ruta
        String[] route = type.getRoute(); // Obtiene la ruta de procesamiento del tipo de entidad
        if (currentStep < route.length) { // Verifica si el paso actual está dentro de la ruta
            return route[currentStep]; // Retorna el nombre de la locación actual
        }
        return null; // Retorna null si no hay más locaciones en la ruta
    }

    public String getNextAlmacen() { // Método que retorna el nombre de la próxima locación (compatible con código existente)
        return getNextLocation(); // Retorna la próxima locación
    }

    public double getCurrentProcessingTime() { // Método que calcula el tiempo de procesamiento del paso actual en minutos
        String[] route = type.getRoute(); // Obtiene la ruta de procesamiento del tipo de entidad
        if (currentStep < route.length) { // Verifica si el paso actual está dentro de la ruta
            String locationName = route[currentStep]; // Obtiene el nombre de la locación actual
            return LOCATION_PROCESS_TIMES.getOrDefault(locationName, 0.0); // Retorna el tiempo de proceso o 0
        }
    ...
    private static Map<String, Double> initializeProcessTimes() { // Método estático privado que inicializa los tiempos de proceso por locación
        Map<String, Double> map = new HashMap<>(); // Crea un nuevo HashMap para almacenar los tiempos
        // Proceso de granos de cebada (en minutos)
        map.put("SILO_GRANDE", 0.0); // Sin procesamiento, solo almacenamiento
        map.put("MALTEADO", 60.0); // 1 hora
        map.put("SECADO", 60.0); // 1 hora
        map.put("MOLIENDA", 60.0); // 1 hora
        map.put("MACERADO", 90.0); // 1.5 horas
        map.put("FILTRADO", 30.0); // 30 minutos
        map.put("COCCION", 60.0); // 1 hora (unión con lúpulo)
        // Proceso de lúpulo
        map.put("SILO_LUPULO", 0.0); // Sin procesamiento
        // Proceso post-cocción
        map.put("ENFRIAMIENTO", 60.0); // 1 hora
        map.put("FERMENTACION", 120.0); // 2 horas (unión con levadura)
        // Proceso de levadura
        map.put("SILO_LEVADURA", 0.0); // Sin procesamiento
        // Proceso final de cerveza
        map.put("MADURACION", 90.0); // 1.5 horas
        map.put("INSPECCION", 30.0); // 30 minutos
        map.put("EMBOTELLADO", 3.0); // 3 minutos
        map.put("ETIQUETADO", 1.0); // 1 minuto
        // Proceso de empacado
        map.put("ALMACEN_CAJAS", 0.0); // Sin procesamiento
        map.put("EMPACADO", 10.0); // 10 minutos (unión de 6 botellas + 1 caja)
        map.put("ALMACENAJE", 5.0); // 5 minutos
        map.put("MERCADO", 0.0); // Salida del sistema
        return map; // Retorna el mapa con los tiempos configurados
    }
}
